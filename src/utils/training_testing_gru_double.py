...
import time
import numpy as np
from utils.model_saver import model_saver_wandb
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            with torch.cuda.amp.autocast():
                ...
                print(batch_races[0].hidden_in.shape)
...
def train_double_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            ...
            t6 = time.perf_counter()
            if not update:
                (2*epoch_loss+epoch_loss_p).mean().backward()
                optimizer.step()
                wandb.log({"loss_1": torch.mean(epoch_loss).item(), 'epoch':epoch}, step = example_ct)
            raceDB.detach_hidden(dogs)
            t7 = time.perf_counter()
...
def test_model_v3(model:GRUNetv3,raceDB:Races,criterion=None, batch_size=None,epoch=10,config=None,device='cuda:0'):
    ...
    with torch.no_grad():
        ...
        onehot_win = F.one_hot(actual, num_classes=20)
        ...
        races['relu'] = [[x]*20 for x in relu.cpu().tolist()]
        # print(races['relu'])
        ...
        races['raceID'] = [[x.raceid]*20 for x in race]
        races['date'] = [[x.race_date]*20 for x in race]
        races['race_num'] = [[int(x.race_num)]*20 for x in race]
        ...
        races['track'] = [[x.track_name]*20 for x in test_races]
